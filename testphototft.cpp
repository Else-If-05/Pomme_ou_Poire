...
#define TFT_MOSI 70 // Data out
#define TFT_SCLK 71  // Clock out
...
void captureImg(uint16_t width, uint16_t height, bool chroma) {
    byte frame[HEIGHT][WIDTH];  // Stockage temporaire de l'image
    uint16_t x, y;
    noInterrupts();
    ...
    while (y--) {
        x = width;
        while (x--) {
            while ((REG_PIOD_PDSR & (1 << 10))); // Attendre PCLK bas
            delayMicroseconds(1);
            frame[y][x] = (REG_PIOC_PDSR & 0xFF000) >> 12; // Lire Y (luminance)
            while (!(REG_PIOD_PDSR & (1 << 10))); // Attendre PCLK haut
        }
    }
    interrupts();
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            uint16_t color = grayscaleToRGB565(frame[y][x]);  // Convertir en RGB565
